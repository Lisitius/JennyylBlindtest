...
// --- Barème, proportionnel à la durée de la manche ---
// Les points restent au maximum pendant le premier cinquième du temps, puis
// diminuent régulièrement jusqu'au minimum atteint à la toute fin. Ainsi le
// rythme est le même en 15 s, 20 s, 30 s ou 45 s.
export function facteurRapidite(secondes, duree = 30) {
  const d = duree > 0 ? duree : 30;
  const repit = d * 0.2;
  if (secondes <= repit) return 10;
  const avancement = Math.min(1, (secondes - repit) / (d - repit));
  return Math.max(1, Math.round(10 - avancement * 9));
}

// Exporté : sert aussi à afficher aux joueurs ce que vaut la manche à cet
// instant, pour qu'ils voient les points fondre et se dépêchent.
export function pointsPartie(secondes, maxPartie, duree = 30) {
  if (!maxPartie) return 0;
  return Math.max(
    1,
    Math.round((facteurRapidite(secondes, duree) / 10) * maxPartie)
  );
}
...
// Ne renvoie QUE les chansons terminées : la manche en cours n'apparaît qu'une
// fois révélée, et les suivantes jamais. Aucun risque de trahir la partie.
export function historiquePublic(salon) {
  const pistes = salon.tracks || [];
  const revele = salon.status === "reveal" || salon.status === "ended";
  const joues = Math.min(pistes.length, (salon.round_index || 0) + (revele ? 1 : 0));
  if (joues <= 0) return [];
  return pistes.slice(0, joues).map((m) => ({
    nom: m.name,
    artistes: m.artists,
    film: m.film || null,
    image: m.image || null,
...
export async function traiterReponse(salon, joueur, texte) {
  ...
  const duree = (fin - debut) / 1000; // durée réelle de la manche
  ...
  if (champs.titre && !trouve.titre) {
    if (
      ...
      const p = pointsPartie(secondes, bareme.titre, duree);
      trouve.titre = true;
      gagnes += p;
  ...
  if (champs.artiste && !trouve.artiste) {
    if (isCorrectAnswer(texte, morceau.artists)) {
      const p = pointsPartie(secondes, bareme.artiste, duree);
      trouve.artiste = true;
      gagnes += p;
  ...
  if (champs.film && !trouve.film) {
    if (
      ...
      const p = pointsPartie(secondes, bareme.film, duree);
      trouve.film = true;
      gagnes += p;
